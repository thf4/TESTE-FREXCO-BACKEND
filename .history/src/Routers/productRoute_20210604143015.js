...
router.post("/user/:_id/product", auth, async (req, res) => {
  ...
  if(!name) return res.status(400).json({ message: "Digite um nome !"})
  if(!description) return res.status(400).json({ message: "Digite uma descrição do produto !"})
  if(!price) return res.status(400).json({ message: "Digite o preço produto, apenas número !"})
  try {
